...
import { Search, RotateCw, Filter, Fingerprint } from "lucide-react"
      ...
      <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-2">
            Browse supporting operational evidence, logs, and artifacts across assets.
          </p>
        </div>
      ...
      <div className="flex flex-col lg:flex-row items-center gap-4 bg-background/40 backdrop-blur-md border rounded-xl p-3 shadow-sm opacity-50 pointer-events-none grayscale-[50%]">
        ...
        <div className="flex flex-wrap items-center gap-3 w-full lg:w-auto">
          <Button variant="outline" className="h-9" disabled>
            <Fingerprint className="mr-2 h-4 w-4" /> Type
          </Button>
          <Button variant="outline" className="h-9" disabled>
            <Filter className="mr-2 h-4 w-4" /> Reference
          </Button>
        </div>
      </div>
    </div>
