...
import { EngineeringChange } from "@/types/models"
import { AnimatedSlideUp } from "@/components/animations/Motion"
import { ChevronDown, ChevronUp, Calendar, User, FileText, CheckCircle, Clock, FilePlus, XOctagon } from "lucide-react"
...
function EngineeringBadge({ status }: { status: EngineeringChange['status'] }) {
...
export function EngineeringCard({ change, delay = 0 }: EngineeringCardProps) {
      ...
      >
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-3 mb-1.5">
            <EngineeringBadge status={change.status} />
            <span className="text-xs text-muted-foreground font-mono">ID: {change.id.slice(0, 8)}</span>
          </div>
          <h4 className="text-base font-semibold text-foreground truncate mb-1">{change.title}</h4>
          {!isExpanded && (
            <p className="text-sm text-muted-foreground truncate">{change.description}</p>
          )}
        </div>
        ...
        <div className="shrink-0 flex items-center gap-4 text-sm text-muted-foreground">
          <div className="hidden md:flex flex-col items-end gap-1">
            <div className="flex items-center gap-1.5">
              ...
              <span>{new Date(change.proposedDate).toLocaleDateString()}</span>
            </div>
            {change.author && (
              <div className="flex items-center gap-1.5">
        ...
        <div className="p-4 pt-0 border-t border-border/50 animate-in fade-in slide-in-from-top-2 duration-200">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mt-4">
            ...
            <div className="space-y-4 bg-muted/20 p-4 rounded-xl border border-border/50 h-fit">
              ...
              <div>
                <span className="text-xs text-muted-foreground block mb-0.5">Proposed Date</span>
                <span className="text-sm font-medium">{new Date(change.proposedDate).toLocaleDateString()}</span>
              </div>
              
              {change.implementationDate && (
                <div>
                  <span className="text-xs text-muted-foreground block mb-0.5">Implementation Date</span>
                  <span className="text-sm font-medium">{new Date(change.implementationDate).toLocaleDateString()}</span>
                </div>
              )}
              
              <div>
                <span className="text-xs text-muted-foreground block mb-0.5">Author / Requester</span>
